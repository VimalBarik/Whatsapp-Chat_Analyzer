...
def plot_sentiment(sentiment_df: pd.DataFrame) -> Optional[plt.Figure]:
    ...
    ax.set_title(f"Sentiment Distribution (Total: {total})")
    return fig
...
def plot_user_engagement(results: Dict) -> List[plt.Figure]:
    """
    Return a list of matplotlib Figures for the engagement metrics.
    (Previously called plt.show() directly; now returns figures so the
    caller — e.g. Streamlit — can render them.)
    """
    figures = []
    ...
    if 'media_analysis' in results and 'data' in results['media_analysis']:
        ...
        if not media_data.empty:
            ...
            ax.set_title("Media Shared per User")
            ...
            if 'percentage' in results['media_analysis']:
                ...
                for i, name in enumerate(media_data['Name']):
                    ...
                    ax.text(
                        i,
                        media_data.loc[media_data['Name'] == name, 'Media_Count'].values[0] + 0.5,
                        f"{pct}%",
                        ha='center'
                    )
    ...
    if 'emoji_analysis' in results and not results['emoji_analysis'].empty:
        ...
        sns.barplot(
            x='Name', y='Avg_Emojis_Per_Message',
            data=results['emoji_analysis'],
            palette=[COLOR_PALETTE['person1'], COLOR_PALETTE['person2']],
            ax=ax2
        )
